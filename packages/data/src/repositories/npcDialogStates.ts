...
export class PostgresNPCDialogStateRepository implements NPCDialogStateRepository {
  ...
  async upsert(state: NPCDialogState): Promise<void> {
    await this.db
      ...
      .onConflict((conflict) => conflict.columns(['sector_name', 'npc_index']).doUpdateSet({ script_step: state.scriptStep }))
      .execute();
  }

  async reset(sectorName: string, npcIndex: number): Promise<void> {
    await this.db.deleteFrom('npc_dialog_states').where('sector_name', '=', sectorName).where('npc_index', '=', npcIndex).execute();
  }
  ...
  async deleteOrphans(keys: readonly NPCDialogStateKey[]): Promise<void> {
    ...
    await this.db.transaction().execute(async (transaction) => {
      for (const key of keys) {
        await transaction.deleteFrom('npc_dialog_states').where('sector_name', '=', key.sectorName).where('npc_index', '=', key.npcIndex).execute();
      }
    });
  }
}
