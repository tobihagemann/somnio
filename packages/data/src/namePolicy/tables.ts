...
export const CONFUSABLES: ReadonlyMap<number, readonly number[]> = parseConfusables(
  confusablesJSON.mappingTable
)
...
const scriptExtensions = new RangeTable(scriptsJSON.scriptExtensionRanges, (field) =>
  field.split(',').map((id) => Number(id))
)
