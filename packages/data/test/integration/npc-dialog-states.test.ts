...
describe('npc dialog state repository', () => {
  ...
  it('deletes only the listed orphans and no-ops on empty input', async () => {
    ...
    expect(
      (await states.allKeys()).sort(
        (a, b) => a.sectorName.localeCompare(b.sectorName) || a.npcIndex - b.npcIndex
      )
    ).toEqual([
    ...
    expect(await states.loadAll('EdariaBibliothek')).toEqual([
      { sectorName: 'EdariaBibliothek', npcIndex: 1, scriptStep: 4 },
    ])
  })
})
