...
const outputDir = explicitOutputDir ?? resolve(fileURLToPath(new URL('../data/name-policy/', import.meta.url)));
...
async function loadSources() {
  ...
  for (const name of SECURITY_FILES) await fetchInto(`https://www.unicode.org/Public/security/${UNICODE_VERSION}/`, name);
  for (const name of UCD_FILES) await fetchInto(`https://www.unicode.org/Public/${UNICODE_VERSION}/ucd/`, name);
  return contents;
}
...
console.log(`script ranges: ${scriptRanges.length}, extension ranges: ${scriptExtensionRanges.length}, scripts: ${sortedScriptNames.length}`);
