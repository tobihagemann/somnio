 ...
 *
 * It lives outside this package and is reached through the `@scripts` alias rather than being
 * relocated: the script is a build-tool sibling of `bundle-web-assets.sh` and belongs beside it.
 ...
 * The shell side propagates a non-zero exit; these cases are what pin the *answers* it gives
...
describe('glb-buffer-uris', () => {
  ...
  describe('readGLTF', () => {
    ...
    it('reads a JSON glTF carrying a .glb extension', () => {
      ...
      const path = write('json.glb', JSON.stringify({ asset: { version: '2.0' }, buffers: [{ uri: 'geo.bin' }] }));
  ...
  describe('externalResourcePaths', () => {
    ...
    it('reports buffers before images', () => {
      ...
      expect(externalResourcePaths({ images: [{ uri: 'skin.png' }], buffers: [{ uri: 'geo.bin' }] })).toEqual(['geo.bin', 'skin.png']);
    });
