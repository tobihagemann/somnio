...
import { SOMNIO_PROTOCOL_CONSTANTS, decodeSomnioMessage, encodeSomnioMessage, MAX_WIRE_FRAME_SIZE } from '@somnio/protocol';
...
/**
 * Wire conformance against a **live gameplay server**.
 *
 * Honest accounting of what each part buys, because they are not equal:
 *
 * - The close-behaviour cases re-test the *server*, which the server package's own live and
 *   integration suites already cover. They are a cheap regression net, not a check on the
...
describe('server handshake conformance', () => {
  ...
  it('sends Hello with the current protocol version on connect', async () => {
    ...
    expect(hello?.tag === 'hello' && hello.payload.protocolVersion).toBe(SOMNIO_PROTOCOL_CONSTANTS.helloVersion);
  });
...
describe('production outbound path', () => {
  ...
  it('sends a valid message through the real transport as a text frame', async () => {
    ...
    expect(say?.text).toBe(encodeSomnioMessage({ tag: 'clientSay', payload: { entityIndex: 0, text: 'conformance' } }));
  });
});
