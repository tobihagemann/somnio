...
/**
 * `GameplayTransport`'s ordering invariants. Each test below pins one of the rationales the
 * transport documents, and those matter more than the code, because every one of them describes
 * a failure that is silent rather than loud.
 */
...
describe('endpoint resolution', () => {
  it('derives wss from an https origin and ws from http', () => {
    expect(resolveGameplayURL({ protocol: 'https:', host: 'somnio.tobiha.de' })).toBe('wss://somnio.tobiha.de/ws');
...
describe('outbox is live before the socket opens', () => {
   ...
   * the client would strand in `awaitingLoginResult` — the exact failure the early outbox exists
   * to prevent.
   */
...
describe('outbound frames are text', () => {
   ...
   * could pass while the transport shipped `ArrayBuffer`, which the server closes on.
   */
...
describe('inbound handling', () => {
  ...
  /**
   * The browser `WebSocket` exposes no `maxFrameSize`, so the inbound cap the server gets from
   * `ws`'s `maxPayload` has to live in the decoder here. This is the one divergence the
  ...
  it('closes on an oversized inbound text frame', () => {
    ...
    latest().deliverText(`{"tag":"adminSay","payload":{"text":"${'a'.repeat(SOMNIO_PROTOCOL_CONSTANTS.maxFrameLength)}"}}`);
...
describe('close handling', () => {
   ...
   * here and no test can catch it. The call is kept anyway: it states the ordering a transport
   * with a real writer queue needs, where frames genuinely are in flight at `disconnect` time and
   * the drain prevents a dropped or misordered frame (`GameplayTransport.disconnect`). The
   * browser's synchronous `send` is what makes the flush unreachable — not a decision that the
 ...
 * live connection. A transport that scoped every per-socket flag to one connection value and
 * refused re-entrancy would be immune by construction; this one has a mutable slot, so it has to
 * guard explicitly.
 */
describe('superseded sockets cannot act on their replacement', () => {
  ...
  it('closes with the protocol-error code even when the delegate disconnects synchronously', () => {
    ...
    // code outside 3000-4999 (1000 aside), so retargeting this at the server's own 1002 would pass every
