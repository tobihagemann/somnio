import { INT16_MAX, INT16_MIN } from '@somnio/core';
import { heading } from '@somnio/core';
import { formatSwiftFloat32 } from '@somnio/core';
import type { Sector } from '@somnio/core';
import { PORTAL_DIRECTIONS } from '@somnio/core';
import type { PortalDirection } from '@somnio/core';
...
export class InspectorPanel {
  ...
  constructor(callbacks: InspectorCallbacks, registry: { objectModelIDs: readonly string[]; floorMaterialIDs: readonly string[] }) {
  ...
  private scriptField(index: number, sector: Sector): DraftField {
    ...
    const row = element('div', {
      children: [
        ...
        textarea,
        caption("Script syntax: --- separates dialog steps; $name substitutes the player's nickname at runtime.").row,
      ],
    });
    return {
      row,
      refresh: (current) => {
        ...
        const seeded = reseeded(textarea.value, globalThis.document.activeElement === textarea, lastRendered, to);
