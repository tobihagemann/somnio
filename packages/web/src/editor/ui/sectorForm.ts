...
import { element, field, select } from '@/ui/dom';
...
export function validationMessage(values: SectorFormValues): string | undefined {
  ...
  if (!Number.isInteger(values.width) || !Number.isInteger(values.height) || !validSectorDimensions(values.width, values.height)) {
...
export class SectorForm {
  ...
  constructor(floorMaterialIDs: readonly string[], onChanged: () => void) {
    ...
    this.root = element('div', {
      children: [name.row, width.row, height.row, indoor.row, brightness.row, floorMaterial.row, this.validationLine],
    });
