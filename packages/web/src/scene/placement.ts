 ...
 * without a renderer. These are the rules that decide whether a prop sits flush with
 ...
 *
 * Narrowed per step because the rotation is Float32 arithmetic throughout, and `FLOAT_PI` rather
 * than `Math.PI` because `FLOAT_PI` is rounded toward zero while `Math.fround(Math.PI)` rounds
 * up — `@somnio/core`'s `float.ts` records that the two land on adjacent `Float` values, so the
