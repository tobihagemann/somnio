 ...
 * metres vertically. The orthographic `scale` is the vertical world half-extent; treating it as
 * full height makes every unprojection overshoot the drawn geometry by exactly 2x from the view
 * centre, and the editor's picking math depends on it.
 *
 ...
 *
 * `PLAYER_ZOOM.scrollGain` is sized for a few delta units per mouse-wheel notch. Chrome and
 * Safari on macOS report ~100 px for the same notch, and
