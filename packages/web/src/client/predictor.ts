 ...
 * to agree with the shared `@somnio/core` geometry exactly, because the *server* re-runs the same
...
/**
 * Module-private, as is `collisionTriggers`: nothing outside this file calls either, and an
 * `export` would suppress `tsc`'s unused-symbol reporting, the only dead-code check the browser
...
/**
 * The player's feet box at the attempted next step
...
/**
 * Peers are always solid; monsters are *soft-solid* —
