...
/**
 * How a connection attempt intends to authenticate.
 *
...
/**
 * The connection half of the client: authentication, session management, and the inbound dispatch.
 *
...
export class ConnectionController {
   ...
   *
   * The history is bounded. Every append triggers a full scrollback rebuild in the DOM
   * (`renderChat` replaces the whole subtree), so an unbounded history makes the cost per message
   * grow with the session — and chat is peer-driven with no server-side rate limit, so one
    ...
    switch (message.tag) {
      ...
      case 'sessionToken':
        ...
        if (this.credentials?.rememberMe === true) {
          // Reported rather than swallowed: the player asked to be remembered, and a
   ...
   *
   * The browser presents the login overlay directly rather than routing through the game menu: it is
