 ...
 *
 * Three ordering invariants, each load-bearing:
 *
 * 1. **The outbox is live before the socket can deliver anything.** The server emits `hello`
 *    immediately on accept and the controller answers it by enqueuing `login` synchronously. If
 *    that enqueue could land before the outbox exists, the auth frame would be silently dropped
 *    and the client would strand in `awaitingLoginResult`. The queue is created in the
 *    constructor and buffers until the socket opens, so there is no window.
 * 2. **The writer drains before the close frame.** `disconnect()` flushes before closing; sending
 *    after close is undefined. The flush is ordering only, and never actually carries a frame:
...
export class GameplayTransport {
   ...
   *
   * The transport has one mutable slot plus callbacks the platform keeps invoking after
   ...
   * letting the receiver hard-close, and a failure is logged
