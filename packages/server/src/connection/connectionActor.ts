...
export type ConnectionState =
  { kind: 'awaitingLogin' } | { kind: 'attached'; entityIndex: number; sectorName: string; accountId: string }
...
export class ConnectionActor {
  ...
  private async driveConnection(socket: ConnectionSocket): Promise<void> {
    ...
    this.logger.info(
      {
        reason:
          ended === 'peerClosed' ? 'peer closed' : ended.kind === 'keepOpen' ? 'server closed' : ended.reason,
        sector: attachedAs,
      },
      'connection closed'
    )
  ...
  private async handleInbound(data: string | Uint8Array, isBinary: boolean): Promise<CloseDecision> {
    ...
    try {
      ...
      this.logger.warn(
        { error: String(error), frame_size: Buffer.byteLength(text, 'utf8') },
  ...
  async dispatch(message: SomnioMessage): Promise<CloseDecision> {
    ...
    switch (message.tag) {
      ...
      case 'bumpNPC':
        ...
        return { kind: 'keepOpen' }
      case 'enterPortal': {
        const outcome = handleEnterPortal(
          message.payload,
          state.entityIndex,
          state.sectorName,
          this,
  ...
  private sendHello(): void {
    this.outbox.sendEncoded(
      { tag: 'hello', payload: { protocolVersion: SOMNIO_PROTOCOL_CONSTANTS.helloVersion } },
      this.logger
    )
  }
