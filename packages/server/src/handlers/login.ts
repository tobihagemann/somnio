...
export async function handleLogin(message: LoginMessage, connection: ConnectionActor, dependencies: ConnectionDependencies): Promise<void> {
  ...
  try {
    ...
    if (account === undefined || !verified) {
      ...
      logger.warn(account === undefined ? { known_account: false } : { known_account: true, name: message.nickname }, 'login rejected: bad credentials');
      sendLoginResult(outbox, LOGIN_RESULT.badCredentials, logger);
      return;
    }
  ...
  try {
    ...
    if (spawn !== undefined) {
      ...
      try {
        ...
        logger.warn({ error: String(error), name: character.name, spawn: `${spawn.x},${spawn.y}` }, 'failed to persist resolved spawn point');
      }
    }
    ...
    try {
      ...
      outbox.sendEncoded({ tag: 'dateTick', payload: dependencies.worldClock.currentDateTickMessage() }, logger);
    } catch (error) {
...
async function issueToken(accountId: string, outbox: ConnectionOutbox, dependencies: ConnectionDependencies): Promise<void> {
  try {
    ...
    outbox.sendEncoded({ tag: 'sessionToken', payload: { token: session.token, expiresInSeconds: remaining } }, dependencies.logger);
  } catch (error) {
...
export function sendLoginResult(outbox: ConnectionOutbox, result: (typeof LOGIN_RESULT)[keyof typeof LOGIN_RESULT], logger: Logger): void {
