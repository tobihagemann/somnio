 ...
 */
export function dispatchAdminRequest(request: AdminRequest, dependencies: AdminDependencies): AdminResponse | undefined {
  switch (request.tag) {
    case 'log':
      return readLog(dependencies.gameplayLog, { tag: 'logEmpty' }, (text) => ({ tag: 'logContents', payload: text }), dependencies.logger);
    case 'weblog':
      return readLog(dependencies.adminLog, { tag: 'weblogEmpty' }, (text) => ({ tag: 'weblogContents', payload: text }), dependencies.logger);
    case 'logRemove':
    ...
    case 'say': {
      ...
      if (text.length === 0 || utf8ByteLength(text) > SOMNIO_PROTOCOL_CONSTANTS.maxSayUTF8Bytes) return undefined;
      dependencies.worldRouter.broadcastToAllConnections({ tag: 'adminSay', payload: { text } });
      return { tag: 'sayBroadcast', payload: text };
    }
    case 'kick': {
      const name = request.payload;
      return dependencies.worldRouter.kickByCharacterName(name) ? { tag: 'kickedPlayer', payload: name } : { tag: 'kickedPlayerNotFound', payload: name };
    }
    case 'version':
...
function readLog(file: AdminLogFile, emptyResponse: AdminResponse, contentsResponse: (text: string) => AdminResponse, logger: Logger): AdminResponse {
  let bytes: Buffer;
  try {
...
function removeLog(file: AdminLogFile, logger: Logger): void {
  if (!file.closeAndRemove()) logger.debug({ file: file.path }, 'admin log close-and-remove reported no file removed');
}
