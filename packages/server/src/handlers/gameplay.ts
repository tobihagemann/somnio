...
import type { BumpNPCMessage, EnterPortalMessage, EquipToggleMessage, PositionMessage, SayMessage } from '@somnio/protocol';
...
export function handlePosition(message: PositionMessage, entityIndex: number, sectorName: string, dependencies: ConnectionDependencies): void {
...
export function handleSay(message: SayMessage, entityIndex: number, sectorName: string, dependencies: ConnectionDependencies): void {
...
export function handleEquipToggle(
  ...
  const rows = dependencies.worldRouter.sector(sectorName)?.handleEquipToggle(message.slot, message.hand, entityIndex);
  if (rows === undefined) return;
  outbox.sendEncoded({ tag: 'inventory', payload: { rows: rows.map(inventoryRowToWire) } }, dependencies.logger);
}

export function handleBumpNPC(message: BumpNPCMessage, entityIndex: number, sectorName: string, dependencies: ConnectionDependencies): void {
...
export function handleEnterPortal(
  ...
  if (portal.direction !== 'outboundTrigger') {
    logger.warn({ direction: portal.direction, portal_index: message.portalIndex, sector: sectorName }, 'enter_portal non-trigger direction');
    oldSector.snapBack(entityIndex);
    return undefined;
  }
  ...
  const position = newSector.arrivalPlacement(sectorName, SOMNIO_CONSTANTS.playerSpriteSize) ?? arrivalSpawn(destination) ?? sectorPixelCenter(destination);
  ...
  try {
    ...
    connection.outbox.sendEncoded({ tag: 'dateTick', payload: dependencies.worldClock.currentDateTickMessage() }, logger);
