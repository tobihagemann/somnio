...
import {
  ...
  rawDataToBuffer,
} from '../connection/connectionActor.ts'
...
export type AdminProcessOutcome =
  | { kind: 'write'; response: AdminResponse }
  | { kind: 'ignore' }
  | { kind: 'closeProtocolError'; reason: string }
      ...
      switch (outcome.kind) {
        case 'write':
          ws.send(encodeAdminResponse(outcome.response), (error) => {
            if (error) {
              dependencies.logger.warn(
                { error: String(error), case: outcome.response.tag },
