...
export class WorldRouter implements AdminWorldRouter {
  ...
  private constructor(sectorActors: Map<string, PerSectorActor>, characters: CharacterRepository, npcDialogStates: NPCDialogStateRepository, logger: Logger) {
    ...
    for (const [sectorName, sector] of this.sectorActors) {
      ...
      for (const state of digest.dialogUpserts) {
        try {
          ...
          this.logger.warn({ error: String(error), sector: sectorName, npc_index: state.npcIndex }, 'npc dialog upsert failed');
        }
      }
      for (const npcIndex of digest.dialogResets) {
        try {
          ...
          this.logger.warn({ error: String(error), sector: sectorName, npc_index: npcIndex }, 'npc dialog reset failed');
        }
      }
    }
  }
