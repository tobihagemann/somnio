...
import { bootTestServer, closeAndAwaitCleanup, frame, joinFreshPlayer, loginOverWire, pollUntil, startDatabase } from './support/harness.ts';
...
describe('inventory round trip', () => {
  ...
  it('equipping a second row to the same hand unequips the first', async () => {
    ...
    expect(first.target.tag === 'inventory' ? first.target.payload.rows.find((row) => row.slot === 0)?.equippedHand : undefined).toBe(WIRE_HAND.right);
    joined.client.send(frame({ tag: 'equipToggle', payload: { slot: 1, hand: WIRE_HAND.right } }));
  ...
  it('equip state is persisted by the close checkpoint and surfaces on reconnect', async () => {
    ...
    expect(await pollUntil(async () => ((await persistedCudgelHand(joined.nickname)) === HAND.right ? true : undefined))).toBe(true);
    ...
    expect(await pollUntil(async () => ((await persistedCudgelHand(joined.nickname)) === undefined ? true : undefined))).toBe(true);
