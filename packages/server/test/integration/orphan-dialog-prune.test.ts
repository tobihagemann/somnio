...
describe('orphan NPC dialog state prune over Postgres', () => {
  it('keeps valid rows, deletes orphans, and logs the summary', async () => {
    ...
    try {
      const sectors = new Map([
        ['A', makeSector('A', { npcs: [makeNPC({ x: 0, y: 0 }, ''), makeNPC({ x: 64, y: 0 }, '')] })],
      ])
  ...
  it('the guard aborts the large prune until forced', async () => {
    ...
    for (let index = 1; index <= 40; index += 1)
      await repo.upsert({ sectorName: 'ghost', npcIndex: index, scriptStep: 1 })
    const logging = tempLogging({ maxBytes: 1 << 20 })
    try {
      await expect(
        pruneOrphanNPCDialogStates(repo, new Map(), false, logging.adminLogger('dialogprune'))
      ).rejects.toBeInstanceOf(DialogPruneGuardError)
