...
describe('NPC dialog end to end', () => {
  it('repeated AI ticks walk the dialog cursor across all script steps then wrap', async () => {
    ...
    const entityIndex = actor.attach(
      makeCharacter(npcRuntimePosition(libus), 'alice', bibliothek.name),
      [],
      outbox
    )
  ...
  it('a bump outside the interaction radius produces no say or dialog upsert', async () => {
    ...
    const entityIndex = actor.attach(
      makeCharacter({ x: runtime.x + 400, y: runtime.y }, 'far', bibliothek.name),
      [],
      outbox
    )
    actor.handleBumpNPC(LIBUS_INDEX, entityIndex)
  ...
  it('the dialog cursor persists across a server restart', async () => {
    ...
    const entityIndex = sector.attach(
      makeCharacter(npcRuntimePosition(libus), 'alice', bibliothek.name),
      [],
      new ConnectionOutbox(4096)
    )
    ...
    for (let step = 0; step < 2; step += 1) {
      for (let tick = 0; tick <= DIALOG_COOLDOWN_CAP; tick += 1)
        await first.worldRouter.runAITickAcrossSectors()
    }
    const persisted = await new PostgresNPCDialogStateRepository(harness.db).find(
      bibliothek.name,
      LIBUS_INDEX
    )
