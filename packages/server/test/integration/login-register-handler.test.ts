...
describe('register handler over Postgres', () => {
  ...
  it('fails when the password repeat does not match', async () => {
    expect(await register(registerMessage(uniqueNickname('alice'), { passwordRepeat: 'passwordB' }))).toEqual(
  ...
  it('fails when the password is shorter than the minimum', async () => {
    expect(
      await register(registerMessage(uniqueNickname('shorty'), { password: 'abc', passwordRepeat: 'abc' }))
    ).toEqual([REGISTER_RESULT.failure])
  })
  ...
  it('fails when the nickname exceeds the identifier length cap', async () => {
    expect(
      await register(registerMessage('n'.repeat(SOMNIO_PROTOCOL_CONSTANTS.maxIdentifierUTF8Bytes + 1)))
    ).toEqual([REGISTER_RESULT.failure])
  })
  ...
  it('fails when the email exceeds the identifier length cap', async () => {
    ...
    expect(await register(registerMessage(uniqueNickname('mail'), { email }))).toEqual([
      REGISTER_RESULT.failure,
    ])
  })
  ...
  it('a malformed class raw returns failure', async () => {
    expect(await register(registerMessage(uniqueNickname('cls'), { characterClass: 42 }))).toEqual([
      REGISTER_RESULT.failure,
    ])
  })
})
...
describe('login handler over Postgres', () => {
  ...
  it('an unknown nickname returns badCredentials', async () => {
    expect(loginResults((await login(uniqueNickname('ghost'))).messages)).toEqual([
      LOGIN_RESULT.badCredentials,
    ])
  })
  ...
  it('a wrong password for an existing account returns badCredentials', async () => {
    ...
    await register(registerMessage(nickname))
    expect(loginResults((await login(nickname, 'not-the-password')).messages)).toEqual([
      LOGIN_RESULT.badCredentials,
    ])
  })
  ...
  it('a second login for the same account returns alreadyLoggedIn', async () => {
    ...
    if (first.connection.state.kind === 'attached')
      dependencies.worldRouter.unregister(first.connection.state.accountId)
  })
  ...
  it('fails when the password exceeds the maximum length', async () => {
    ...
    expect(loginResults((await login(uniqueNickname('longpw'), password)).messages)).toEqual([
      LOGIN_RESULT.badCredentials,
    ])
  })
