...
describe('gameplay handshake over a live socket', () => {
  ...
  it.each([
    ...
    [
      'an unknown tag of 300 bytes',
      (client: TestClient) => client.send(`{"tag":"${'x'.repeat(300)}","payload":{}}`),
    ],
    [
      'a recognized tag with a malformed payload',
      (client: TestClient) => client.send('{"tag":"clientPosition","payload":{}}'),
    ],
