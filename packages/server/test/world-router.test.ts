...
async function routerOver(sectors: Map<string, ReturnType<typeof makeSector>>, dialogRepo = new RecordingDialogRepository()) {
...
describe('WorldRouter.runAITickAcrossSectors', () => {
  ...
  it('init pre-loads the persisted cursor from the repository', async () => {
    ...
    const router = await routerOver(new Map([['A', sectorWithScript('A', 'first line.\n---\nsecond line.')]]), dialogRepo);
  ...
  it('persists a non-final dialog upsert through the repository', async () => {
    const dialogRepo = new RecordingDialogRepository();
    const router = await routerOver(new Map([['A', sectorWithScript('A', 'first.\n---\nsecond.\n---\nthird.')]]), dialogRepo);
    bump(router, 'A', 'alice');
    await router.runAITickAcrossSectors();
