...
async function world(
  options: { clock?: WorldClock; sourcePortals?: SectorPortal[]; destinationPortals?: SectorPortal[] } = {}
) {
...
function moved(outcome: ReturnType<typeof handleEnterPortal>): PortalOutcome {
  if (outcome === undefined || outcome === PORTAL_LOST)
    throw new Error(`portal hop did not move: ${String(outcome)}`)
  return outcome
}
...
describe('handleEnterPortal', () => {
  ...
  it('places the player at the destination arrival spawn', async () => {
    ...
    const entityIndex = attachAt(w, { x: 1, y: 1 })
    const outcome = moved(
      handleEnterPortal({ portalIndex: 0 }, entityIndex, 'A', w.connection, w.dependencies)
    )
    expect(outcome.sectorName).toBe('B')
    expect(w.actorB.snapshotForPlayer(outcome.entityIndex)?.character.position).toEqual(
      arrivalSpawn(w.actorB.staticSector)
    )
  })
  ...
  it('recenters an out-of-bounds carry into a sector without an arrival portal', async () => {
    ...
    expect(w.actorB.snapshotForPlayer(outcome.entityIndex)?.character.position).toEqual(
      sectorPixelCenter(w.actorB.staticSector)
    )
  })
  ...
  it('rejects a non-outbound-trigger direction and snaps back', async () => {
    ...
    const entityIndex = attachAt(w, { x: 1, y: 1 })
    expect(
      handleEnterPortal({ portalIndex: 1 }, entityIndex, 'A', w.connection, w.dependencies)
    ).toBeUndefined()
    expect((await collectMessages(w.connection.outbox)).map((message) => message.tag)).toContain(
      'serverPosition'
    )
  ...
  it('rejects an out-of-range index and snaps back', async () => {
    ...
    const entityIndex = attachAt(w, { x: 1, y: 1 })
    expect(
      handleEnterPortal({ portalIndex: 5 }, entityIndex, 'A', w.connection, w.dependencies)
    ).toBeUndefined()
    expect((await collectMessages(w.connection.outbox)).map((message) => message.tag)).toContain(
      'serverPosition'
    )
