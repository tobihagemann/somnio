...
describe('handleRegister validation', () => {
  it('a mismatched password repeat fails before the repository', async () => {
    ...
    expect(await registerResult({ ...valid, passwordRepeat: 'other-password' }, repository)).toEqual([
  ...
  it('a short password fails', async () => {
    expect(await registerResult({ ...valid, password: 'short', passwordRepeat: 'short' })).toEqual([
      REGISTER_RESULT.failure,
    ])
  })
