 ...
 */
export async function makeStubConnectionDependencies(
  options: StubDependencyOptions = {}
  ...
  const worldRouter =
    options.worldRouter ??
    (await WorldRouter.create(
      options.sectors ?? new Map(),
      characters,
      new StubNPCDialogStateRepository(),
      logger
    ))
  const worldClock = new WorldClockService(
    worldRouter,
    new StubWorldClockRepository(),
    options.initialClock ?? BOOT_DEFAULT_WORLD_CLOCK,
    logger
  )
  return {
