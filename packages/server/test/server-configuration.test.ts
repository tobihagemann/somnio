...
describe('resolveServerConfiguration', () => {
  ...
  it('no admin token without dev defaults throws missingAdminToken', () => {
    expect(kindOf(() => resolveServerConfiguration({ SOMNIO_SECTORS_DIR: '/srv/maps' }))).toBe(
      'missingAdminToken'
    )
  })
  ...
  it('no sectors dir without dev defaults throws missingSectorsDirectory', () => {
    expect(kindOf(() => resolveServerConfiguration({ SOMNIO_ADMIN_TOKEN: 'secret' }))).toBe(
      'missingSectorsDirectory'
    )
  })

  it('non-numeric port throws invalidPort', () => {
    expect(
      kindOf(() => resolveServerConfiguration({ SOMNIO_DEV_DEFAULTS: '1', SOMNIO_HTTP_PORT: 'not-a-port' }))
    ).toBe('invalidPort')
  })

  it.each(['0', '65536', '-1', '70000', '1.5'])('out-of-range port %s throws invalidPort', (raw) => {
    expect(
      kindOf(() => resolveServerConfiguration({ SOMNIO_DEV_DEFAULTS: '1', SOMNIO_HTTP_PORT: raw }))
    ).toBe('invalidPort')
  })

  it('empty admin token without dev defaults is rejected as missing', () => {
    expect(
      kindOf(() => resolveServerConfiguration({ SOMNIO_ADMIN_TOKEN: '', SOMNIO_SECTORS_DIR: '/srv/maps' }))
    ).toBe('missingAdminToken')
  })

  it.each(['1', 'true', 'TRUE', 'True'])(
    'truthy SOMNIO_DIALOG_PRUNE_FORCE=%s resolves forceDialogPrune true',
    (raw) => {
      const configuration = resolveServerConfiguration({
  ...
  it.each(['', '0', 'false', 'no', 'yes'])(
    'non-truthy SOMNIO_DIALOG_PRUNE_FORCE=%j resolves forceDialogPrune false',
    (raw) => {
      const configuration = resolveServerConfiguration({
        SOMNIO_DEV_DEFAULTS: '1',
        SOMNIO_DIALOG_PRUNE_FORCE: raw,
