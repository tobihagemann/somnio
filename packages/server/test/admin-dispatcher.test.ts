...
import {
  LOG_WIRE_LIMIT_BYTES,
  dispatchAdminRequest,
  truncateToWireLimit,
} from '../src/handlers/adminDispatcher.ts'
...
describe('say', () => {
  it('broadcasts and returns sayBroadcast', async () => {
    const router = new StubAdminWorldRouter()
    const response = dispatchAdminRequest(
      { tag: 'say', payload: 'hello' },
      await dependencies({ worldRouter: router })
    )
  ...
  it('empty text returns undefined and records no broadcast', async () => {
    const router = new StubAdminWorldRouter()
    expect(
      dispatchAdminRequest({ tag: 'say', payload: '' }, await dependencies({ worldRouter: router }))
    ).toBeUndefined()
  ...
  it('text over the wire cap returns undefined and records no broadcast', async () => {
    ...
    expect(
      dispatchAdminRequest({ tag: 'say', payload: oversized }, await dependencies({ worldRouter: router }))
    ).toBeUndefined()
    expect(router.broadcasts).toEqual([])
  })
})
...
describe('kick / version', () => {
  it('kick of a match returns kickedPlayer', async () => {
    ...
    expect(
      dispatchAdminRequest({ tag: 'kick', payload: 'Saibot' }, await dependencies({ worldRouter: router }))
    ).toEqual({
      tag: 'kickedPlayer',
      payload: 'Saibot',
    })
