...
function inside(point: { x: number; y: number }, portal: SectorPortal): boolean {
  return (
    point.x >= portal.x &&
    point.x < portal.x + portal.width &&
    point.y >= portal.y &&
    point.y < portal.y + portal.height
  )
}
...
describe('arrivalPlacement', () => {
  it('lands inside the inbound portal keyed to the source sector', () => {
    const portal = makePortal(
      { x: 1344, y: 208, width: 160, height: 96 },
      'EdariaBibliothek',
      'arrivalPlacement'
    )
  ...
  it('returns undefined when every cell is blocked', () => {
    ...
    expect(
      actorWith('EdariaMitte', portal, [mask], 4).arrivalPlacement('EdariaBibliothek', SPRITE)
    ).toBeUndefined()
  })
})
