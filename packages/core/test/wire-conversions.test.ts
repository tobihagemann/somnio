...
describe('sector conversions', () => {
  ...
  it('accepts dimensions at the boundary', () => {
    ...
    const wire = sectorToWire(makeSector({ dimensions: { width: SOMNIO_CONSTANTS.maxSectorDimension, height } }));
  ...
  it.each([
    ...
    expect(() => sectorToWire(makeSector({ dimensions: { width, height } }))).toThrow(expect.objectContaining({ kind: 'sectorDimensionsOutOfRange' }));
  });
  ...
  it.each([
    ...
    [
      ...
      {
        objects: repeated(SOMNIO_CONSTANTS.maxSectorObjects, OBJECT),
        collisionMasks: repeated(SOMNIO_CONSTANTS.maxSectorAnchorScanPairings / SOMNIO_CONSTANTS.maxSectorObjects + 1, MASK),
      },
    ],
  ] as const)('refuses %s over the cap on the way out', (_label, overrides) => {
    expect(() => sectorToWire(makeSector(overrides))).toThrow(expect.objectContaining({ kind: 'sectorContentCountsOutOfRange' }));
  });
