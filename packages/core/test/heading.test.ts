 ...
 *
 * Every expectation below is a recorded Float32 result, not re-derived in JavaScript, and every
 * case is one where binary32 and binary64 actually
...
describe('FLOAT_PI is rounded toward zero, not fround', () => {
  /**
   * `FLOAT_PI` is pi rounded toward zero; `Math.fround` rounds to nearest, which for pi
