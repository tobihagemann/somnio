...
function portal(x: number, y: number, width: number, height: number, targetSectorName: string, direction: SectorPortal['direction']): SectorPortal {
...
describe('EdariaMitte', () => {
  ...
  it('carries the walled square', () => {
    ...
    expect(sector.objects.filter((object) => object.modelID === 'stone-wall' && object.rotation === 90)).toHaveLength(32);
    ...
    expect(sector.objects.some((object) => object.modelID === 'building-house' && object.rotation === 270)).toBe(true);
    expect(sector.objects.some((object) => object.modelID === 'building-house' && object.rotation === 0)).toBe(true);
...
describe('Nordwiese', () => {
  it('is the meadow with the town wall along its south border', () => {
    ...
    expect(sector.floorPatches).toEqual([{ floorMaterialID: 'cobble-town', x: 512, y: 1408, width: 512, height: 128 }]);
...
describe('Nordwald', () => {
  it('is the pine forest with one ghost spawn', () => {
    ...
    expect(sector.npcs).toEqual([]);
    expect(sector.portals).toEqual([portal(640, 1520, 256, 16, 'Nordwiese', 'outboundTrigger'), portal(688, 1360, 160, 96, 'Nordwiese', 'arrivalPlacement')]);
...
describe('EdariaShop', () => {
  it('is the compact Kramladen', () => {
    ...
    expect(sector.portals).toEqual([portal(86, 238, 84, 18, 'EdariaMitte', 'outboundTrigger'), portal(72, 148, 112, 72, 'EdariaMitte', 'arrivalPlacement')]);
  });
});
...
describe('EdariaInn', () => {
  it('is the compact Gaststube', () => {
    ...
    expect(sector.objects.filter((object) => object.modelID === 'counter' && object.rotation === 90)).toHaveLength(2);
    expect(sector.objects.filter((object) => object.modelID === 'bed' && object.rotation === 270)).toHaveLength(2);
    ...
    expect(sector.portals).toEqual([portal(238, 86, 18, 84, 'EdariaMitte', 'outboundTrigger'), portal(144, 96, 72, 80, 'EdariaMitte', 'arrivalPlacement')]);
  });
});
...
describe('EdariaArena', () => {
  it('is the stone arena with one ghost spawn', () => {
    ...
    expect(sector.npcs).toEqual([]);
    expect(sector.portals).toEqual([portal(494, 166, 18, 180, 'EdariaMitte', 'outboundTrigger'), portal(384, 176, 96, 160, 'EdariaMitte', 'arrivalPlacement')]);
    expect(sector.monsterSpawns).toHaveLength(1);
    const gespenst = sector.monsterSpawns[0]!;
    expect(gespenst.name).toBe('Gespenst');
