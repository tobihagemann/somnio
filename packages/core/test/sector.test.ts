...
describe('portal direction', () => {
   ...
   * with the same constant, so a swapped pair round-trips cleanly through the codec tests, the
   * golden frames, and every case that spells the names symbolically — and only shows up in
   * play, as a sector whose arrival area is treated as one giant portal trigger that blocks every
...
describe('arrivalSpawn', () => {
   ...
   * The scan steps 8 px; a browser that scanned on a finer lattice would stand an arriving
   * player somewhere the server does not expect.
   */
