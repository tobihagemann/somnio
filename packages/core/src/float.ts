/**
 * Single-precision narrowing. Headings, the yaw slew, and the camera rig are Float32 quantities:
 * the committed golden frames and sector fixtures carry Float32 values, and the browser and the
 * server must agree on them. JavaScript numbers are IEEE binary64 throughout, and narrowing only
 * at the wire boundary is not enough — an intermediate double carries extra mantissa bits into
 * the next operation, so a multi-step expression drifts even though every input and output is
...
/**
 * Single-precision pi, rounded toward zero.
 *
 * This is deliberately **not** `Math.fround(Math.PI)`. The fixtures were produced with pi
 * rounded *toward zero* (which keeps angle computations from tipping into the wrong quadrant),
 * while `Math.fround` rounds to nearest, which for pi rounds *up*. The two land on adjacent
 * `Float` values:
 *
 *     Math.fround(Math.PI) === 3.1415927410125732   // to nearest
 *     FLOAT_PI             === 3.141592502593994    // toward zero
 *
...
/**
 * Single-precision truncating remainder. JavaScript `%` already truncates toward zero with the
...
/**
 * Single-precision IEEE 754 remainder, which rounds the quotient to
 ...
 *
 * `NPC.facing` is the **only** `Float` anywhere in the on-disk sector
 * model — every other field is `Int16`/`String`/`Bool` — so this exists solely for the
 * `"direction"` key. A heading is in `[0, 360)`, so the one place Foundation and JS pick different
