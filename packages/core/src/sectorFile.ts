 ...
 *
 * The writer reproduces the format's Foundation-era output (pretty-printed, sorted keys,
 * unescaped slashes): 2-space indent, `"key" : value`
 ...
 * The committed fixtures (plus the synthetic encoding golden) pin this writer to that output
 * byte for byte.
 *
...
/**
 * TypeScript's `number` is unrestricted where the format's integers are `Int16`, so both directions
 * check every persisted integer — an inspector that accepted `1.5` or `40000` would otherwise
 * write a file `readSectorFile` rejects, surfacing the failure at server startup instead of
 * at save.
 */
...
/**
 * Guard order: dimensions, content counts, then — after serializing — the UTF-8 byte cap, so
 * the writer can never persist a file its own reader would refuse.
 */
