 ...
 * converts to and from `string` at the frame edge, never `ArrayBuffer` — the server closes on
 * any binary frame.
 */
...
/**
 * Decodes one inbound text frame. Rejected: malformed JSON, a non-object frame, a non-string
 * discriminator, an unknown tag, a known tag whose payload fails validation, and a frame over
 * `maxFrameLength`. The size cap lives here
 * rather than only at the WebSocket layer because the browser `WebSocket` exposes no
