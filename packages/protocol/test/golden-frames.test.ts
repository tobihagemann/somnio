...
const committed = JSON.parse(readFileSync(GOLDEN_FRAMES_PATH, 'utf8')) as Record<string, { tag: string; payload: unknown }>;
...
function recordedFrames(): Record<string, unknown> {
  return Object.fromEntries(GOLDEN_FRAME_ENTRIES.map((entry) => [entry.name, JSON.parse(encodeSomnioMessage(entry.message))]));
}
...
describe('golden frames', () => {
  ...
  it('encoder output matches the committed fixtures', () => {
    ...
    for (const entry of GOLDEN_FRAME_ENTRIES) {
      ...
      expect(expected, `missing fixture "${entry.name}" — re-record with SOMNIO_RECORD_GOLDEN_FRAMES=1`).toBeDefined();
      expect(canonicalize(recorded[entry.name]), `golden frame "${entry.name}" drifted`).toEqual(canonicalize(expected));
    }
  });

  it.each(Object.keys(committed).sort())('%s decodes through the real decoder and re-encodes unchanged', (name) => {
