export type SecureTransportValidationErrorKind =
  'invalidURL' | 'unsupportedScheme' | 'insecureRemoteURL' | 'userinfoNotAllowed'
...
export function validateHostAgreement(url: string): void {
  ...
  if (parsed.hostname !== rawAuthorityHost(url).toLowerCase())
    throw new SecureTransportValidationError('invalidURL')
}
