...
function buildCommandTree(onVerb: (verb: RoutedVerb) => Promise<void>, output: CommanderOutput): Command {
  ...
  const withServerURL = (command: Command) =>
    command.option('--server-url <url>', 'Admin WebSocket URL (env: SOMNIO_ADMIN_URL).')
  // Accepted before the verb as well as after it; the verb's own value wins when both are given.
  withServerURL(program)
  const verb = (request: AdminRequest, command: Command) =>
    onVerb({ request, serverURL: command.optsWithGlobals<{ serverUrl?: string }>().serverUrl })
  ...
  withServerURL(log.command('rm').description('Delete the gameplay log.')).action(
    (_options, command: Command) => verb({ tag: 'logRemove' }, command)
  )
  ...
  withServerURL(weblog.command('rm').description('Delete the admin log.')).action(
    (_options, command: Command) => verb({ tag: 'weblogRemove' }, command)
  )
  withServerURL(program.command('players').description('Show the number of logged-in players.')).action(
    (_options, command: Command) => verb({ tag: 'players' }, command)
  )
  withServerURL(program.command('time').description('Show the in-game world clock.')).action(
    (_options, command: Command) => verb({ tag: 'time' }, command)
  )
  ...
  withServerURL(program.command('version').description('Show the server version.')).action(
    (_options, command: Command) => verb({ tag: 'version' }, command)
  )
  return program
}
