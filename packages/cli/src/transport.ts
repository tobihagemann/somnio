...
export class AdminTransportError extends Error {
  ...
  constructor(kind: AdminTransportErrorKind, cause?: unknown) {
    super(cause === undefined ? kind : `${kind}: ${cause instanceof Error ? cause.message : JSON.stringify(cause)}`);
...
export function dialableURL(url: string): string {
  try {
    ...
    if (error instanceof SecureTransportValidationError) throw new AdminTransportError('invalidTransportURL', error);
    throw error;
  }
  return url;
}
...
export function send(request: AdminRequest, url: string, token: string, trust: TrustRootsResolution = resolveTrustRoots()): Promise<AdminResponse> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('unexpected-response', (_request, incoming) => {
      settle({
        error: new AdminTransportError('connectFailed', new Error(`upgrade rejected with ${incoming.statusCode}`)),
      });
      socket.terminate();
    });
