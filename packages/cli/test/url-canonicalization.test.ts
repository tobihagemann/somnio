...
describe('dialableURL', () => {
  ...
  it.each(['wss://☃.example/admin', 'wss://ex%41mple.com/admin', 'ws://[::1]:8080/admin', 'ws://localhost#evil.com/admin'])(
    'rejects the host-disagreeing URL %s',
    (url) => {
      let caught: unknown;
      try {
        dialableURL(url);
...
describe('validate', () => {
  ...
  it('accepts every loopback spelling over ws and any host over wss', () => {
    for (const url of ['ws://localhost/admin', 'ws://127.0.0.1/admin', 'ws://LOCALHOST:1/admin', 'wss://any.example/admin', 'wss://Admin.Example.com/admin']) {
