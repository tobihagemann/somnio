...
async function expectTransportError(kind: AdminTransportError['kind'], body: () => Promise<unknown>): Promise<AdminTransportError> {
  let caught: unknown;
  try {
    await body();
  } catch (error) {
...
async function withScriptedServer(handler: (ws: WebSocket) => void, body: (url: string) => Promise<void>): Promise<void> {
...
describe('send', () => {
  ...
  it('fails with connectFailed when the bearer is refused', async () => {
    ...
    cleanups.push(() => adminDependencies.logging.cleanup());
    await withLiveServer({ adminDependencies }, async (server) => {
      const error = await expectTransportError('connectFailed', () => send({ tag: 'players' }, adminURL(server), 'wrong'));
  ...
  it('surfaces connectFailed when the server is unreachable', async () => {
    await expectTransportError('connectFailed', () => send({ tag: 'players' }, 'ws://127.0.0.1:1/admin', 'tok'));
  });

  it.each(['ws://attacker@localhost:8080/admin', 'WSS://localhost/admin', 'ws://example.com/admin', ''])('rejects %j before opening a socket', async (url) => {
  ...
  it('rejects a validate-passing but host-disagreeing URL before dialing', async () => {
    await expectTransportError('invalidTransportURL', () => send({ tag: 'players' }, 'wss://ex%41mple.com/admin', 'tok'));
  });

  it('fails closed with pinningRefused on a wss dial with refused roots', async () => {
    await expectTransportError('pinningRefused', () => send({ tag: 'players' }, 'wss://example.com:443/admin', 'tok', { kind: 'refused', reason: 'bad pem' }));
  });

  it('a wss dial with pinned roots fails on connect, not on validation', async () => {
    const error = await expectTransportError('connectFailed', () => send({ tag: 'players' }, 'wss://127.0.0.1:1/admin', 'tok', { kind: 'pinned', ca: [] }));
