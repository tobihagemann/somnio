...
describe('resolveAdminConnection', () => {
  ...
  it('wss URLs are accepted regardless of host', () => {
    expect(resolveAdminConnection('wss://prod.example/admin', { SOMNIO_ADMIN_TOKEN: 'tok' }).url).toBe(
      'wss://prod.example/admin'
    )
  })

  it('loopback plaintext URLs are accepted', () => {
    expect(resolveAdminConnection('ws://127.0.0.1:8080/admin', { SOMNIO_ADMIN_TOKEN: 'tok' }).url).toBe(
      'ws://127.0.0.1:8080/admin'
    )
  })
  ...
  it('an empty token counts as unset', () => {
    expect(resolveAdminConnection('ws://localhost:8080/admin', { SOMNIO_ADMIN_TOKEN: '' }).token).toBe(
      DEV_ADMIN_TOKEN
    )
