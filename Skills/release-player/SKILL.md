...
- **Drop non-player changes.** Editor-only work (map authoring, object/floor pickers, sector-file save/open) does not ship to players — the web editor is dev-only and never released. Server-only work releases separately via `/release-server` and has no changelog. Neither belongs here.
...
- The server and browser client release separately: the server via the `/release-server` skill, the browser client via the `/release-web` skill. The web editor is dev-only and never released.
