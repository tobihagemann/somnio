...
- Keep the display awake before posting synthetic input: an asleep display drops CGEvents without error. Start `caffeinate -u -d` first, then send keys. `peekaboo press <key> --foreground` and `peekaboo type "<text>" --foreground` do deliver keystrokes — both report `success: false` with `effect: "unverifiable"` while still dispatching, so confirm the result by observing the app rather than by the return value. Without `--foreground` they are refused outright.
