---
name: somnio-player
description: "Build and launch the native macOS player client locally against the dev server, for local play and testing. Use when the user asks to run, play, launch, or try the native player or the macOS app, or to test a gameplay change in the running app. For the browser client, use the somnio-web skill instead; for production releases, use the release-player skill instead."
---
...
- The login overlay's **Log In** button (the registration sheet's is **Sign Up**) exposes no AX-pressable element (`peekaboo see` intermittently times out on the game window) and `peekaboo click --foreground` is blocked by the missing AppleScript automation permission, but a **window-relative coordinate click** does press it: `peekaboo click --coords <x>,<y> --app Somnio`. Read the coordinates straight off a `peekaboo image` capture of the same window — they are window-relative and land where the capture shows them, with no scaling. Seed `credential.json` first so the fields are pre-filled, since no text field accepts synthetic input.
...
- Synthetic movement keys reach the game only after a mouse click into the 3D world view makes it first responder — right after the login click, focus still sits on the **Log In** button and held keys are silently ignored (0 movement, no error).
