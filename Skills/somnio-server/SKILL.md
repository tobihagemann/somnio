...
Convention: a `somnio-pg` container (postgres:16) on port 5432.

```bash
docker ps --filter name=somnio-pg
# if absent:
docker run -d --name somnio-pg -p 5432:5432 -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=somnio postgres:16
```

If port 5432 is taken, map an alternate host port (e.g. `-p 5433:5432`) and adjust `SOMNIO_DATABASE_URL` in Step 3. On teardown use `docker stop somnio-pg` — keep the container; `docker rm`-ing it drops the dev account/character, and the next login then shows "Bad credentials" against the fresh empty DB.
...
Always rebuild before (re)starting — a running server keeps the old code in memory after a binary change.
...
```bash
SOMNIO_DATABASE_URL=postgres://postgres:postgres@localhost:5432/somnio \
