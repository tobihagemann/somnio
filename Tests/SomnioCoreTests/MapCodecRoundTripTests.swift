...
struct MapCodecRoundTripTests {
    ...
    func `npc direction byte survives a write then read unchanged`(direction: Int16) throws {
        // `NPC.direction` carries the original `richtung` encoding verbatim; the codec is
        // byte-faithful for it (semantic conversion happens at the server emit seam, not here).
        let body = SectorBody(
            version: 1,
            dimensions: GridSize(width: 4, height: 4),
            ground: GroundTile(tilesetIndex: 0, sourceX: 0, sourceY: 0),
