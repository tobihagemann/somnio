...
/// Localized-string access for the UI module. `Bundle.somnioUIModule` resolves to the
...
enum L {
    static func string(_ key: String.LocalizationValue, locale: Locale? = nil) -> String {
        if let locale {
            return String(localized: key, bundle: .somnioUIModule, locale: locale)
        }
        return String(localized: key, bundle: .somnioUIModule)
    }
    ...
    static func resource(_ key: String.LocalizationValue) -> LocalizedStringResource {
        LocalizedStringResource(key, bundle: .atURL(Bundle.somnioUIModule.bundleURL))
    }
}
