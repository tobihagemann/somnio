...
/// Localized-string access for the player client. `Bundle.somnioAppModule` resolves to the
...
enum L {
    static func string(_ key: String.LocalizationValue, locale: Locale? = nil) -> String {
        if let locale {
            return String(localized: key, bundle: .somnioAppModule, locale: locale)
        }
        return String(localized: key, bundle: .somnioAppModule)
    }
    ...
    static func resource(_ key: String.LocalizationValue) -> LocalizedStringResource {
        LocalizedStringResource(key, bundle: .atURL(Bundle.somnioAppModule.bundleURL))
    }
}
